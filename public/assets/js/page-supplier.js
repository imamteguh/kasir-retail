document.addEventListener("DOMContentLoaded", function () {
  ...
  let supplierTable = document.querySelector(".datatables-suppliers");

  // Inisialisasi DataTable
  if (supplierTable) {
    let dt = new DataTable(supplierTable, {
      ajax: { url: "/api/suppliers", dataSrc: "data" },
      columns: [
      ...
      columnDefs: [
        ...
        // Phone
        {
          ...
          render: (data, type, row) => {
            return `<span class="text-heading">${row.phone || "-"}</span>`;
          }
        },
        // Address
        {
          ...
          render: (data, type, row) => {
            return `<span class="text-heading">${row.address || "-"}</span>`;
          }
        },
        // Actions
        {
          ...
          render: (data, type, row) => `
            <div class="d-flex align-items-center gap-2">
              <a href="javascript:;" class="btn btn-icon btn-edit" 
                data-id="${row.id}" 
                data-name="${row.name}"
      ...
      layout: {
        ...
        topEnd: {
          features: [
            { search: { placeholder: "Search Supplier", text: "_INPUT_" } }
          ]
        },
  ...
  $(document).on('click', '.btn-delete', function (e) {
    ...
    title.innerHTML = `Delete Supplier ${name} !!`;
  ...
  $('#editSupplierForm').on('submit', function (e) {
      ...
      $.ajax({
        ...
        success: function(response) {
          if (response.success) {
            ...
            toastError(response.message || 'Failed to update supplier');
    ...
    if (editSupplierValidator) {
      editSupplierValidator.validate().then(function (status) {
        if (status === 'Valid') {
          runAjaxUpdate();
        }
      });
  ...
  // Inisialisasi FormValidation untuk form Add Supplier
  let addSupplierForm = document.querySelector("#addNewSupplierForm");
  let addSupplierValidator;
  if (addSupplierForm && typeof FormValidation !== "undefined") {
    addSupplierValidator = FormValidation.formValidation(addSupplierForm, {
      fields: {
        name: {
          validators: {
            notEmpty: { message: "Please enter supplier name" },
  ...
  // Save data
  $('#addNewSupplierForm').on('submit', function (e) {
    e.preventDefault();
    const button = $(this).find('button[type="submit"]');
    const form = this;
    const $form = $(form);
      ...
      $.ajax({
        ...
        success: function(response) {
          if (response.success) {
            ...
            toastError(response.message || 'Failed to save supplier');
    ...
    if (addSupplierValidator) {
      addSupplierValidator.validate().then(function (status) {
        if (status === 'Valid') {
          runAjaxSave();
        }
